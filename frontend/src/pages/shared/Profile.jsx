  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 24, alignItems: 'start' }}>
        ...
        <div>
          ...
          <div className="tabs" style={{ flexDirection: 'column', borderBottom: 'none', borderRight: '2px solid var(--border)' }}>
            {['profile', 'password', 'notifications'].map(t => (
              <button key={t} className={`tab-btn ${tab === t ? 'active' : ''}`} style={{ textAlign: 'left', borderBottom: 'none', borderRight: '2px solid transparent', marginRight: -2 }}
                onClick={() => setTab(t)}>
        ...
        {/* Right Column */}
        <div className="card">
          <div className="card-header"><h3>{tab === 'profile' ? 'Profile Information' : tab === 'password' ? 'Change Password' : 'Notification Preferences'}</h3></div>
          <div className="card-body">
            ...
            {tab === 'notifications' && (
              <form onSubmit={handleSave}>
                ...
                {[['email', '📧 Email Notifications', 'Receive updates via email'], ['push', '📱 Push Notifications', 'Receive browser push notifications'], ['inApp', '🔔 In-App Notifications', 'See notifications inside the platform']].map(([key, label, desc]) => (
                ...
                <button type="submit" className="btn btn-primary mt-4" disabled={loading}>{loading ? 'Saving...' : 'Save Preferences'}</button>
              </form>
            )}
          </div>
        </div>
