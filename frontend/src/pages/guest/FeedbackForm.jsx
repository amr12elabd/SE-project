...
const FeedbackForm = () => {
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', padding: 24 }}>
      <div style={{ background: '#fff', borderRadius: 20, maxWidth: 480, width: '100%', overflow: 'hidden', boxShadow: '0 8px 40px rgba(0,0,0,0.12)', textAlign: 'center' }}>
        <div style={{ background: 'linear-gradient(135deg, #1a6b5c, #2d9b87)', padding: '36px 32px 28px', color: '#fff' }}>
          ...
          <h2 style={{ margin: 0, fontWeight: 800 }}>{wasUpdated ? 'Review Updated!' : t('thankYouFeedback')}</h2>
          <p style={{ margin: '8px 0 0', opacity: 0.85 }}>{t('feedbackSubmitted')}</p>
        </div>
  ...
  return (
    <div>
      <div className="page-header"><h1>⭐ {t('submitFeedback')}</h1></div>

      {eventOptions.length === 0 ? (
        <div className="empty-state">
          <div className="empty-state-icon">⭐</div>
          <h3>{t('noEventsToRate')}</h3>
          <p>{t('feedbackAvailableFor')}</p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} style={{ maxWidth: 600 }}>
          <div className="card card-body mb-4">
            <div className="form-group">
              <label className="form-label">{t('selectEvent2')} *</label>
              <select
          ...
          <div className="card card-body mb-4">
            <h3 className="mb-4">{t('rateExperience')}</h3>
            <StarRating label={t('overallExperience') + ' *'} value={form.overallRating} onChange={v => setForm(f => ({ ...f, overallRating: v }))} />
            <StarRating label={t('foodBeverages')} value={form.foodRating} onChange={v => setForm(f => ({ ...f, foodRating: v }))} />
            <StarRating label={t('venueAmbience')} value={form.venueRating} onChange={v => setForm(f => ({ ...f, venueRating: v }))} />
            <StarRating label={t('organization')} value={form.organizationRating} onChange={v => setForm(f => ({ ...f, organizationRating: v }))} />
          </div>
