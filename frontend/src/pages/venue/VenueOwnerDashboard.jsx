  ...
  return (
    <div>
      ...
      <div className="grid-4 mb-6">
        <DashboardCard icon="🏛️" label={t('myVenues')} value={venues.length} color="#1a6b5c" />
        <DashboardCard icon="📥" label={t('bookings')} value={pendingBookings.length} color="#dd6b20" />
        <DashboardCard icon="✅" label={t('confirmedBookings')} value={approvedBookings.length} color="#38a169" />
        <DashboardCard icon="⭐" label={t('avgRatingVenue')} value={avgRating} color="#d4875a" />
      </div>
        ...
        <div className="card">
          ...
          {venues.length === 0 ? (
            <div className="empty-state" style={{ padding: 32 }}>
              ...
              <button className="btn btn-primary btn-sm mt-3" onClick={() => navigate('/venue/new')}>{t('addNewVenue')}</button>
            </div>
          ) : venues.map(v => (
            <div key={v._id} style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div className="text-sm text-muted">👥 {t('capacity')}: {v.capacity} · ⭐ {v.rating?.toFixed(1) || 'New'}</div>
              </div>
      ...
      <div className="card card-body mt-4">
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          {[
            { label: '🏛️ ' + t('myVenues'), to: '/venue/listings' },
            { label: t('addNewVenue'), to: '/venue/new' },
            { label: '📥 ' + t('bookings'), to: '/venue/bookings' },
            { label: '✅ ' + t('confirmedBookings'), to: '/venue/confirmed' },
            { label: '📊 ' + t('reports'), to: '/venue/reports' },
          ].map(a => (
